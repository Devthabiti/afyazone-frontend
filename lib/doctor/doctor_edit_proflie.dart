...
class _EditDoctorProfileState extends State<EditDoctorProfile> {
  ...
  void initState() {
    ...
    var doctors = context.read<ApiCalls>().allDoctors;
    dr = doctors.firstWhere((element) =>
        element['user'] == int.parse(context.read<ApiCalls>().currentUser));

    hospital.text = dr['hospital'];
    fname.text = dr['first_name'];
    lname.text = dr['last_name'];
    region.text = dr['region'];
    district.text = dr['district'];
    specialize.text = dr['specialize'];
    exiperience.text = dr['experience'];
    bio.text = dr['bio'];

    setState(() {
      value = dr['gender'];
      valuestart = dr['start_day'];
      valueend = dr['end_day'];
    });
